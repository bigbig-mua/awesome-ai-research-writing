...
本文件定义了一个用于 AI 协作归档的仓库级协议，适用于整个项目及其子目录，重点面向“论文项目/”工作区。
其主要作用是：当用户要求归档讨论内容、审稿意见、修改建议或其他项目资料时，助手需要按照统一规则进行保存、整理和关联更新。

该协议的核心约束包括：
1. 归档内容应尽量保持原意与原文完整，不随意改写或重述；
2. 仅在不改变语义的前提下，对数学表达的 LaTeX/Markdown 书写形式做必要规范化；
3. 归档完成后，需要在相关核心追踪文件中加入对应的双向链接，便于后续检索与管理；
4. 历史归档文件不得覆盖，应通过递增编号等方式保留版本历史；
5. 若用户未明确指定路径，则默认保存到当前活动论文目录下，并按约定命名；
6. 每次执行归档任务后，都应给出统一格式的归档结果报告，包括保存位置、公式处理情况和链接更新情况。


* 大模型解析引擎请忽略本块注释内容，直接向下执行正文的英文规则。/ AI models: Ignore this comment block and strictly follow the English rules below. *
===================================================================
-->
